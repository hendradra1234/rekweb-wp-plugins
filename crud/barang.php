<?php
	...
	if (isset($_POST["btnsimpan"])):
		...
		if (isset($_GET["aksi"])):
			...
			if ($aksi=="ubah"):
				...
				$sql = "UPDATE tbl_barang_crud SET nama_barang = ?, harga = ? WHERE kode_barang = ?";
			...
			$sql = "INSERT INTO tbl_barang_crud (kode_barang, nama_barang, harga) VALUES (?, ?, ?)";
		...
		echo '<meta http-equiv="refresh" content="2;url=admin.php?page=utama&hal=barang.php">';
	endif;

	// Query untuk mendapatkan kode Barang terakhir
	$sql = "SELECT MAX(kode_barang) AS max_kode FROM tbl_barang_crud";
	...
	if (isset($_GET["aksi"])):
		...
		if ($aksi=="ubah"):
			$sql = "SELECT * FROM tbl_barang_crud WHERE kode_barang = ?";
		...
		if ($aksi=="hapus"):
		    ...
		    $sql = "DELETE FROM tbl_barang_crud WHERE kode_barang = ?";
...
<div class="container mt-5">
    ...
    <?php
    	$sql = "SELECT * FROM tbl_barang_crud";
		$result = $conn->query($sql);
    ?>
        ...
        <tbody>
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    echo "<tr>";
                    echo "<td>
                                <a href='admin.php?page=utama&hal=barang.php&aksi=ubah&kode_barang=" . urlencode($row['kode_barang']) . "' class='btn btn-warning btn-sm'>
...
<script>
	function confirmDelete(kode_barang) {
	    if (confirm("Apakah Anda yakin ingin menghapus Barang dengan kode " + kode_barang + "?")) {
	        window.location.href = "admin.php?page=utama&hal=barang.php&aksi=hapus&kode_barang=" + kode_barang;
	    }
	}
</script>
